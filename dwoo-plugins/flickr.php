...
{
    $cacheKey = sprintf('flickr?tagmode=%s&tags=%s', urlencode((string) $tagmode), urlencode($tags));

    if (!$flickrData = apc_fetch($cacheKey)) {
        $feedURL = sprintf('http://api.flickr.com/services/feeds/photos_public.gne?format=php_serial&tagmode=%s&tags=%s', urlencode((string) $tagmode), urlencode($tags));
