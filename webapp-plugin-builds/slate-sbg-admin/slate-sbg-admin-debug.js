Ext.define('Slate.sbg.admin.overrides.SectionTermReport', {override:'Slate.model.progress.SectionTermReport'}, function(Report) {
  Report.addFields([{name:'SbgWorksheet', convert:function(v) {
    return typeof v == 'string' ? Ext.decode(v, true) || null : v;
  }}]);
});
Ext.define('Slate.sbg.admin.store.StandardsWorksheetPromptOptions', {extend:'Ext.data.Store', config:{fields:['id', 'text'], data:[{id:0, text:'N/A'}, {id:1, text:'1'}, {id:2, text:'2'}, {id:3, text:'3'}, {id:4, text:'4'}]}});
Ext.define('Slate.sbg.admin.widget.WorksheetPrompt', {extend:'Ext.container.Container', xtype:'sbg-worksheets-prompt', requires:['Ext.form.field.ComboBox'], config:{prompt:null, grade:null}, layout:'hbox', items:[{xtype:'combobox', width:60, submitValue:false, store:{xclass:'Slate.sbg.admin.store.StandardsWorksheetPromptOptions'}, valueField:'id', displayField:'text', queryMode:'local', forceSelection:true, autoSelect:true, matchFieldWidth:false}, {itemId:'promptCmp', flex:1, xtype:'component', tpl:'{Prompt}'}], 
...
Ext.define('Slate.sbg.admin.overrides.SectionTermReportEditorForm', {override:'SlateAdmin.view.progress.terms.EditorForm', requires:['Ext.form.FieldSet', 'Slate.sbg.admin.widget.WorksheetPrompt'], initComponent:function() {
  var me = this;
...
Ext.define('Slate.sbg.admin.store.StandardsWorksheetPrompts', {extend:'Ext.data.Store', model:'Slate.sbg.admin.model.StandardsWorksheetPrompt', config:{pageSize:false, autoSync:false, sorters:[{property:'Position'}]}});
Ext.define('Slate.sbg.admin.view.worksheets.Grid', {extend:'Ext.grid.Panel', xtype:'sbg-worksheets-grid', requires:['Ext.grid.plugin.CellEditing', 'Ext.form.field.Text', 'Ext.grid.column.Action'], title:'Available Worksheets', componentCls:'sbg-worksheets-grid', bbar:[{flex:1, itemId:'createWorksheetBtn', xtype:'button', text:'Create Worksheet', glyph:61525}], hideHeaders:true, store:'StandardsWorksheets', columns:[{flex:1, text:'Title', dataIndex:'Title', emptyCellText:'Untitled worksheet'}, {xtype:'actioncolumn', 
width:35, align:'end', items:[{action:'delete', iconCls:'prompt-delete glyph-danger', glyph:61526, tooltip:'Delete prompt'}]}]});
Ext.define('Slate.sbg.admin.view.worksheets.PromptsGrid', {extend:'Ext.grid.Panel', xtype:'sbg-worksheets-promptsgrid', requires:['Ext.grid.plugin.CellEditing', 'Ext.form.field.TextArea', 'Ext.grid.column.Action'], title:'Prompts', componentCls:'sbg-worksheets-promptsgrid', plugins:[{ptype:'cellediting', pluginId:'cellediting', clicksToEdit:1}], hideHeaders:true, store:'StandardsWorksheetPrompts', columns:[{text:'Position', width:40, dataIndex:'Position'}, {flex:1, text:'Prompt', dataIndex:'Prompt', 
editor:{xtype:'textarea', allowBlank:false, maxLength:500, enforceMaxLength:true, enterIsSpecial:true}}, {xtype:'actioncolumn', width:65, align:'end', items:[{action:'up', glyph:61538, tooltip:'Move prompt up'}, {action:'down', glyph:61539, tooltip:'Move prompt down'}, {action:'delete', iconCls:'prompt-delete glyph-danger', glyph:61526, tooltip:'Delete prompt'}]}]});
Ext.define('Slate.sbg.admin.view.worksheets.Form', {extend:'Ext.form.Panel', xtype:'sbg-worksheets-form', requires:['Ext.form.field.Text', 'Ext.form.field.TextArea', 'Slate.sbg.admin.view.worksheets.PromptsGrid'], disabled:true, title:'Selected Worksheet', componentCls:'sbg-standards-worksheets-editor', bodyPadding:10, scrollable:'vertical', trackResetOnLoad:true, defaults:{labelWidth:70, labelAlign:'right', anchor:'100%'}, items:[{xtype:'textfield', name:'Title', fieldLabel:'Title', allowBlank:false}, 
{xtype:'textareafield', name:'Description', fieldLabel:'Description', emptyText:'Optional explanation of worksheet for parents and students', grow:true}, {xtype:'sbg-worksheets-promptsgrid'}], buttons:[{itemId:'revertBtn', text:'Revert Changes', cls:'glyph-danger', glyph:61527}, {xtype:'tbfill'}, {itemId:'addPromptBtn', text:'Add Prompt', glyph:61525}, {itemId:'saveBtn', text:'Save Changes', cls:'glyph-success', glyph:61528}]});
Ext.define('Slate.sbg.admin.view.worksheets.Manager', {extend:'Ext.Container', xtype:'sbg-worksheets-manager', requires:['Slate.sbg.admin.view.worksheets.Grid', 'Slate.sbg.admin.view.worksheets.Form'], componentCls:'sbg-worksheets-manager', layout:'border', items:[{region:'west', split:true, xtype:'sbg-worksheets-grid', autoScroll:true, width:500}, {region:'center', xtype:'sbg-worksheets-form', flex:1}]});
Ext.define('Slate.sbg.admin.controller.Worksheets', {extend:'Ext.app.Controller', routes:{'settings/standards-worksheets':'showWorksheetSettings'}, views:['worksheets.Manager@Slate.sbg.admin.view'], stores:['StandardsWorksheets@Slate.sbg.store', 'StandardsWorksheetPrompts@Slate.sbg.admin.store'], refs:{settingsNavPanel:'settings-navpanel', managerCt:{selector:'sbg-worksheets-manager', autoCreate:true, xtype:'sbg-worksheets-manager'}, grid:'sbg-worksheets-grid', form:'sbg-worksheets-form', titleField:'sbg-worksheets-form field[name\x3dTitle]', 
...
Ext.define('Slate.sbg.admin.overrides.SlateAdmin', {override:'SlateAdmin.Application', requires:['Slate.sbg.admin.controller.Worksheets', 'Slate.sbg.admin.controller.SectionTermReports'], initControllers:function() {
  this.callParent();
  this.getController('Slate.sbg.admin.controller.Worksheets');
  this.getController('Slate.sbg.admin.controller.SectionTermReports');
}});
Ext.define('Slate.sbg.admin.overrides.TermReportPrintContainer', {override:'SlateAdmin.view.progress.terms.print.Container', config:{standardsWorksheetTitle:'Standards Worksheet'}, initComponent:function() {
  var me = this;
  me.callParent(arguments);
  me.down('fieldset#includeFieldset').add({boxLabel:me.getStandardsWorksheetTitle(), name:'print[sbg_worksheet]', checked:true});
}});
Ext.define('Slate.sbg.admin.overrides.TermReportSectionsGrid', {override:'SlateAdmin.view.progress.terms.SectionsGrid', requires:['Ext.grid.plugin.CellEditing', 'Ext.form.field.ComboBox'], width:300, worksheetColumnTitle:'Worksheet', emptyWorksheetText:'\x3cem\x3eDouble-click to select\x3c/em\x3e', initComponent:function() {
  var me = this;
